...
On the management side of things, technology manager Rajesh Sharma and Disney Animation CTO, [Nick Cannon](https://twitter.com/ncannon?lang=en), provided crucial support and encouragement.
...
It's taken nearly a year from announcement to release because the process has been complex, and it was very important to the studio to make sure the release was done properly.

One of the biggest challenges was getting all of the data out of the production pipeline and our various proprietary data formats into something that the research community can actually parse and make use of.
...
For example, Hyperion subdivides/tessellates/displaces everything to as close to sub-poly-per-pixel as it can get while still fitting within computational resources.
...
The Cloud Data Set includes an example render from Hyperion, which implements our Spectral and Decomposition Tracking paper in its volumetric rendering system to efficiently render the cloud with thousands of bounces.
...
Also provided is an example [Mitsuba](https://www.mitsuba-renderer.org) scene, renderable using the [Mitsuba-VDB plugin that can be found on Github](https://github.com/zhoub/mitsuba-vdb).
